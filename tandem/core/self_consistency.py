"""
Self-consistency solution generation for the Tandem framework.

This module implements the self-consistency approach, which generates multiple
solutions to a problem via different prompting strategies, then uses voting
to select the most consistent answer.
"""
...
def extract_final_answer(full_solution: str, problem_type: str = None) -> str:
    ...
    Args:
        ...
        problem_type: Optional problem type for specialized extraction
...
def generate_diverse_answers(problem: str, model: str, num_answers: int = 5, problem_category: str = None) -> List[str]:
    ...
    Args:
        ...
        problem_category: Optional problem category for specialized generation
    ...
    if problem_category and problem_category.lower() in ['math', 'puzzles']:
        ...
        solver_params = get_optimal_parameters("math")
        ...
        solver_params = get_optimal_parameters("self_consistency_solver")
    ...
    delete_model(solver_model_name)
    print(f"\n=== Model Cleanup ===")
...
def select_answer_by_voting(answers: List[str], problem: str, model: str, problem_category: str = None) -> Tuple[str, Dict[str, Any]]:
    ...
    Args:
        answers: List of complete solution texts
        problem: The original problem text
        model: Model to use for verification
        problem_category: Optional problem category for specialized extraction
        
    Returns:
        Tuple of (selected_solution, metadata_dict)
    """
    if not answers:
    ...
    for i, full_solution in enumerate(answers):
        if extract_final_answer(full_solution, problem_category) == most_common_answer:
...
def verify_answer_correctness(selected_answer: str, problem: str, model: str) -> Tuple[bool, str]:
    """
    Verify the correctness of the selected answer using a specialized verification model.
    
    Args:
        selected_answer: The answer to verify
        problem: The original problem
        model: The model to use as base
    ...
    # Clean up the temporary model
    delete_model(verification_model_name)
...
def self_consistency_solve(problem: str, model: str, num_answers: int = 5, 
    ...
    """
    Solve a problem using the self-consistency approach with multiple solutions and voting.
    
    Args:
        problem: The problem to solve
        model: The model to use
        num_answers: Number of solutions to generate
        use_meta_cognitive: Whether to use meta-cognitive feedback for low confidence answers
        confidence_threshold: Threshold for applying meta-cognitive feedback
        meta_max_iterations: Maximum iterations of meta-cognitive feedback
    ...
    # Get problem category from the classifier for consistent classification
    from tandem.models.manager import classify_problem
    problem_category = classify_problem(problem)
    
    # Generate diverse solutions with the classification
